...
export const createProfessional = async (req, res) => {
  ...
  try {
    ...
    const newProfessional = await Professional.create({
      role, specialties: specialties || [], commissionRate: commissionRate || 0,
      empresa_id: empresaId, login_id: newLogin.id
    }, { transaction: t });
    await t.commit();
...
export const updateMySchedule = async (req, res) => {
  try {
    const loginId = req.user.id;
    const { workingHours } = req.body; // Espera receber um objeto JSON com os horários

    // Encontra o perfil profissional associado ao login
    ...
    // Atualiza o campo 'workingHours' e salva
...
/**
 * @desc    Profissional logado atualiza seus horários bloqueados.
 * @route   PUT /api/profissionais/my-blocked-slots
 * @access  Private (Professional)
 */
export const updateBlockedSlots = async (req, res) => {
  try {
    const loginId = req.user.id;
    const { blockedSlots } = req.body; // Espera receber um array de objetos JSON
    ...
    // Atualiza o campo 'blockedSlots' e salva
